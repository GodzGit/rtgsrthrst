...
$userId = $_SESSION['admin_id']; // Ambil ID admin dari sesi
...
<style>
    ...
    /* CSS untuk membuat tabel komentar discrollable */
    #hidden-comments {
        max-height: 300px;
        overflow-y: auto;
    }
...
<div class="section">
    <div class="container">
        ...
        <div class="box">
            ...
            <div class="col-2">
                <!-- Tautan untuk mendownload gambar -->
                <a href="foto/<?php echo $p->image ?>" download>
                    <button type="button" class="button-download">Download Gambar</button>
                </a>

                <h3>Komentar</h3>
    <?php
$komentarQuery = mysqli_query($conn, "SELECT * FROM komentarfoto WHERE image_id = '".$_GET['id']."' ORDER BY tanggalkomentar DESC");
$totalKomentar = mysqli_num_rows($komentarQuery);

if ($totalKomentar > 0) {
    echo '<div id="comments-container">'; // Tambahkan id pada kontainer komentar
    while ($komentar = mysqli_fetch_assoc($komentarQuery)) {
        $adminId = $komentar['admin_id'];
        $adminQuery = mysqli_query($conn, "SELECT admin_name FROM tb_admin WHERE admin_id = '$adminId'");
        $adminData = mysqli_fetch_assoc($adminQuery);
        
        echo "<div class='comment'><strong>" . $adminData['admin_name'] . "</strong>: " . $komentar['isikomentar'] . "
                <a href='delete-comment.php?id=" . $komentar['id'] . "'>Hapus</a>
              </div>";
    }
    echo '</div>'; // Tambahkan penutup kontainer komentar
} else {
